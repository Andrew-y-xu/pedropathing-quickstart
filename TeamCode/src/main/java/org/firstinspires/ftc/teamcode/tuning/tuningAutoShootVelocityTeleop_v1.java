...
import org.firstinspires.ftc.teamcode.common.AutoShooter;
import org.firstinspires.ftc.teamcode.common.AutoShooter_UseVelocity;

import java.util.ArrayList;
import java.util.List;

@TeleOp(name="Tuning AutoShootVelocityTeleop_v1")
public class tuningAutoShootVelocityTeleop_v1 extends OpMode {

    //*** Shooter
    //DcMotor shooterFlywheelMotor;
    DcMotorEx shooterFlywheelMotor;
...
//    final AutoShooter autoShoot = new AutoShooter();
    ...
    */
    ...
    public void init() {
        ...
        shooterFlywheelMotor = hardwareMap.get(DcMotorEx.class, "testmotor");
        shooterFlywheelMotor.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        shooterFlywheelMotor.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
        shooterFlywheelMotor.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.FLOAT);
        ...
        */
        ...
        if (motorDebounce.milliseconds() > 300) {
            if (gamepad1.x && !xAlrPressed) {
                ...
                autoShoot.setFlywheelVelocity(shooter_value);
                motorDebounce.reset();
            }
        ...
        shooterFlywheelMotor.setVelocity(shooter_value);
...
//                Double TxValue = resultsofpooe.getTx();
        ...
        //shooterPowerValue = autoShoot.getFlywheelPower();  //--- Update Shooter FlyWheel math here, or use new shooter class for object
        shooterVelocityValue = autoShoot.getFlywheelVelocity();
        //shooterFlywheelMotor.setPower(shooterPowerValue);
        shooterFlywheelMotor.setVelocity(shooterVelocityValue);
        //--- Shooter Angle
        servoPositionValue = autoShoot.getAnglePosition();  //--- Update Shooter Angle math here, or use new shooter class for object
        shooterHoodServo.setPosition(servoPositionValue);
...
//        }
        ...
        if (motorDebounce.milliseconds() > 300) {
            if (gamepad1.x && !xAlrPressed) {
                ...
                shooterVelocityValue += tuningShooterIncrementConstant;
                motorDebounce.reset();
            }
