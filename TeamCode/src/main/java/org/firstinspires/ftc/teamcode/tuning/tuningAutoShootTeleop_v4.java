...
public class tuningAutoShootTeleop_v4 extends OpMode {
    ...
    static double liftservoUpPosition = 0.58;
    static double liftservoDownPosition = 0.05;

    static double liftservo2UpPosition = 0.47;
    static double liftservo2DownPosition = 0.98;
    static double liftservo3UpPosition = 0.63;
    static double liftservo3DownPosition = 0.12;
...
//    Servo convey;
    ...
    public void init() {
        ...
        liftservo = hardwareMap.get(Servo.class, "lift2");  //Left
        lift2servo = hardwareMap.get(Servo.class, "lift1"); //Right
        lift3servo = hardwareMap.get(Servo.class, "lift3"); //Back
...
//        convey = hardwareMap.get(Servo.class, "convey");
        ...
        liftservo.setPosition(liftservoDownPosition);
        lift2servo.setPosition(liftservo2DownPosition);
        lift3servo.setPosition(liftservo3DownPosition);
...
//        }
        ...
        //*** Triple flickers cycle tuning
        if (gamepad1.dpad_up  && !fullCycleRunning) {
            fullCycleRunning = true; fullCycleStartTime = timer.milliseconds();
        }
        ...
        if (cycleRunning) {
            ...
            if (cycleMode == 1) {
                if (t < 300) liftservo.setPosition(liftservoUpPosition); //0.55
                else if (t < 500) liftservo.setPosition(liftservoDownPosition); //0.05
                else { cycleRunning = false; cycleMode = 0; }
            }
            else if (cycleMode == 2) {
                if (t < 300) lift2servo.setPosition(liftservo2UpPosition); //0.30
                else if (t < 500) lift2servo.setPosition(liftservo2DownPosition);  //0.745
                else { cycleRunning = false; cycleMode = 0; }
            }
            else if (cycleMode == 3) {
                if (t < 300) lift3servo.setPosition(liftservo3UpPosition);
                else if (t < 500) lift3servo.setPosition(liftservo3DownPosition);
                else { cycleRunning = false; cycleMode = 0; }
            }
        }
